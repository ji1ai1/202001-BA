...
訓練參照表 = rbind(
	氣入追加表[氣入追加日序 >= 測試日序 - 7 & 氣入追加日序 < 測試日序, .(會員標識, 拍賣標識, 標籤 = 1)]
	, 入札表[入札日序 >= 測試日序 - 7 & 入札日序 < 測試日序, .(會員標識, 拍賣標識, 標籤 = 2)]
)
訓練參照表 = 訓練參照表[, .(標籤 = max(標籤)), .(會員標識, 拍賣標識)]
訓練會員表 = unique(訓練參照表[, .(會員標識)], by = NULL)

訓練氣入追加表 = 氣入追加表[氣入追加日序 < 測試日序 - 7]
訓練入札表 = 入札表[入札日序 < 測試日序 - 7]
訓練落札表 = 落札表[落札日序 < 測試日序 - 7]
...
取得資料 = function(某會員表, 某氣入追加表, 某入札表, 某落札表, 某日序)
{
	...
	候選表 = merge(候選表, 拍賣表[, .(拍賣標識, 商品種別標識, 商品標識, 再出品回數, 狀態標識, 品牌標識, 類型標識, 類型組標識, 線標識, 男女別標識)], by = "拍賣標識")
...
訓練資料表 = 取得資料(訓練會員表, 訓練氣入追加表, 訓練入札表, 訓練落札表, 測試日序 - 7)
訓練資料表 = merge(訓練資料表, 氣入追加表[氣入追加日序 >= 測試日序 - 7 & 氣入追加日序 < 測試日序, .(氣入追加標籤 = 1), .(會員標識, 拍賣標識)], by = c("會員標識", "拍賣標識"), all.x = T)
訓練資料表 = merge(訓練資料表, 入札表[入札日序 >= 測試日序 - 7 & 入札日序 < 測試日序, .(入札標籤 = 1), .(會員標識, 拍賣標識)], by = c("會員標識", "拍賣標識"), all.x = T)
訓練資料表[is.na(訓練資料表)] = 0
訓練資料表 = cbind(訓練資料表[, .(會員標識, 拍賣標識, 氣入追加標籤, 入札標籤)], 訓練資料表[, -c("會員標識", "拍賣標識", "氣入追加標籤", "入札標籤")])

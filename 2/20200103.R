...
訓練資料表 = 取得資料(訓練會員表, 訓練氣入追加表, 訓練入札表, 測試日序 - 7)
訓練資料表 = merge(訓練資料表, 氣入追加表[氣入追加日序 >= 測試日序 - 7 & 氣入追加日序 < 測試日序, .(氣入追加標籤 = 1), .(會員標識, 拍賣標識)], all.x = T)
訓練資料表 = merge(訓練資料表, 入札表[入札日序 >= 測試日序 - 7 & 入札日序 < 測試日序, .(入札標籤 = 1), .(會員標識, 拍賣標識)], all.x = T)
訓練資料表[is.na(訓練資料表)] = 0

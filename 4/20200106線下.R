...
落札表 = fread("rakusatsu.csv", col.names=c("拍賣標識", "會員標識", "落札日", "金額", "數量", "削除標記"))
落札表$落札日序 = as.double(as.Date(substring(落札表$落札日, 1, 10)) - as.Date("2019-10-01"))
落札表 = merge(落札表, 會員表, by = "會員標識")
...
for (子 in 1:4)
{
	print(子)
	線下訓練參照表 = rbind(
		線下氣入追加表[氣入追加日序 >= 線下測試日序 - 7 * 子 & 氣入追加日序 < 線下測試日序 - 7 * 子 + 7, .(會員標識, 拍賣標識, 標籤 = 1)]
		, 線下入札表[入札日序 >= 線下測試日序 - 7 * 子 & 入札日序 < 線下測試日序 - 7 * 子 + 7, .(會員標識, 拍賣標識, 標籤 = 2)]
	)
	線下訓練參照表 = 線下訓練參照表[, .(標籤 = max(標籤)), .(會員標識, 拍賣標識)]
	線下訓練會員表 = unique(線下訓練參照表[, .(會員標識)], by = NULL)

	線下訓練氣入追加表 = 線下氣入追加表[氣入追加日序 < 線下測試日序 - 7 * 子]
	線下訓練入札表 = 線下入札表[入札日序 < 線下測試日序 - 7 * 子]
	線下訓練落札表 = 線下落札表[落札日序 < 線下測試日序 - 7 * 子]


	線下訓練資料表子 = 取得資料(線下訓練會員表, 線下訓練氣入追加表, 線下訓練入札表, 線下訓練落札表, 線下測試日序 - 7 * 子)
	線下訓練資料表子 = merge(線下訓練資料表子, 線下氣入追加表[氣入追加日序 >= 線下測試日序 - 7 * 子 & 氣入追加日序 < 線下測試日序 - 7 * 子 + 7, .(氣入追加標籤 = 1), .(會員標識, 拍賣標識)], by = c("會員標識", "拍賣標識"), all.x = T)
	線下訓練資料表子 = merge(線下訓練資料表子, 線下入札表[入札日序 >= 線下測試日序 - 7 * 子 & 入札日序 < 線下測試日序 - 7 * 子 + 7, .(入札標籤 = 1), .(會員標識, 拍賣標識)], by = c("會員標識", "拍賣標識"), all.x = T)
	線下訓練資料表子[is.na(線下訓練資料表子)] = 0
	線下訓練資料表子 = cbind(線下訓練資料表子[, .(會員標識, 拍賣標識, 氣入追加標籤, 入札標籤)], 線下訓練資料表子[, -c("會員標識", "拍賣標識", "氣入追加標籤", "入札標籤")])
	線下訓練資料表 = rbind(線下訓練資料表, 線下訓練資料表子)
}
...
線下測試資料表 = 取得資料(線下測試會員表, 線下氣入追加表, 線下入札表, 線下落札表, 線下測試日序)
...
線下預測表 = 線下預測表[, .(預測序 = 1:20, 拍賣標識 = c(unique(拍賣標識[!is.na(拍賣標識)]), 線下補充表$拍賣標識)[1:20]), .(會員標識)]
